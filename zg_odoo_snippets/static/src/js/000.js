...
for (let i = 0; i < elements.length; i++) {
    ...
    function getWebGLContext(e) {
        ...
        t || (r = e.getContext("webgl", n) || e.getContext("experimental-webgl", n)),
            t ? (r.getExtension("EXT_color_buffer_float"),
                o = r.getExtension("OES_texture_float_linear")) :
            (i = r.getExtension("OES_texture_half_float"), o = r.getExtension("OES_texture_half_float_linear")),
            r.clearColor(0, 0, 0, 1);
        const a = t ? r.HALF_FLOAT : i.HALF_FLOAT_OES;
        let u, v, l;
        return t ? (u = getSupportedFormat(r, r.RGBA16F, r.RGBA, a), v = getSupportedFormat(r, r.RG16F, r.RG, a),
            l = getSupportedFormat(r, r.R16F, r.RED, a)) : (u = getSupportedFormat(r, r.RGBA, r.RGBA, a),
            v = getSupportedFormat(r, r.RGBA, r.RGBA, a), l = getSupportedFormat(r, r.RGBA, r.RGBA, a)), {
            gl: r,
            ext: {
                formatRGBA: u,
                formatRG: v,
    ...
    class o {
        constructor(e, n) {
            if (this.uniforms = {}, this.program = t.createProgram(), t.attachShader(this.program, e),
                t.attachShader(this.program, n), t.linkProgram(this.program), !t.getProgramParameter(this.program, t.LINK_STATUS))
    ...
    const a = compileShader(t.VERTEX_SHADER, "\n    precision highp float;\n\n    attribute vec2 aPosition;\n    varying vec2 vUv;\n    varying vec2 vL;\n    varying vec2 vR;\n    varying vec2 vT;\n    varying vec2 vB;\n    uniform vec2 texelSize;\n\n    void main () {\n        vUv = aPosition * 0.5 + 0.5;\n        vL = vUv - vec2(texelSize.x, 0.0);\n        vR = vUv + vec2(texelSize.x, 0.0);\n        vT = vUv + vec2(0.0, texelSize.y);\n        vB = vUv - vec2(0.0, texelSize.y);\n        gl_Position = vec4(aPosition, 0.0, 1.0);\n    }\n"),
        ...
        x = (() => (t.bindBuffer(t.ARRAY_BUFFER, t.createBuffer()),
            t.bufferData(t.ARRAY_BUFFER, new Float32Array([-1, -1, -1, 1, 1, 1, 1, -1]),
                t.STATIC_DRAW), t.bindBuffer(t.ELEMENT_ARRAY_BUFFER, t.createBuffer()),
            t.bufferData(t.ELEMENT_ARRAY_BUFFER, new Uint16Array([0, 1, 2, 0, 2, 3]),
                t.STATIC_DRAW), t.vertexAttribPointer(0, 2, t.FLOAT, !1, 0, 0),
            t.enableVertexAttribArray(0), e => {
    ...
    function initFramebuffers() {
        ...
        E = n.width, S = n.height,
            D = r.width, y = r.height;
        const o = i.halfFloatTexType,
            a = i.formatRGBA,
            u = i.formatRG,
            v = i.formatR;
        A = createDoubleFBO(2, D, y, a.internalFormat, a.format, o, i.supportLinearFiltering ? t.LINEAR : t.NEAREST),
            _ = createDoubleFBO(0, E, S, u.internalFormat, u.format, o, i.supportLinearFiltering ? t.LINEAR : t.NEAREST),
    ...
    function createFBO(e, n, r, i, o, a, u) {
        ...
        return t.bindFramebuffer(t.FRAMEBUFFER, l),
            t.framebufferTexture2D(t.FRAMEBUFFER, t.COLOR_ATTACHMENT0, t.TEXTURE_2D, v, 0),
            t.viewport(0, 0, n, r), t.clear(t.COLOR_BUFFER_BIT), {
                texture: v,
                fbo: l,
                texId: e
            }
    ...
    function step(n) {
        t.disable(t.BLEND), t.viewport(0, 0, E, S), M.bind(), t.uniform2f(M.uniforms.texelSize, 1 / E, 1 / S),
            t.uniform1i(M.uniforms.uVelocity, _.read.texId), x(b.fbo), G.bind(),
            t.uniform2f(G.uniforms.texelSize, 1 / E, 1 / S), t.uniform1i(G.uniforms.uVelocity, _.read.texId),
            t.uniform1i(G.uniforms.uCurl, b.texId), t.uniform1f(G.uniforms.curl, e.CURL), t.uniform1f(G.uniforms.dt, n),
            x(_.write.fbo), _.swap(), O.bind(), t.uniform2f(O.uniforms.texelSize, 1 / E, 1 / S),
            t.uniform1i(O.uniforms.uVelocity, _.read.texId), x(F.fbo), L.bind();
        let r = U.read.texId;
        t.activeTexture(t.TEXTURE0 + r),
            t.bindTexture(t.TEXTURE_2D, U.read.texture), t.uniform1i(L.uniforms.uTexture, r),
            t.uniform1f(L.uniforms.value, e.PRESSURE_DISSIPATION), x(U.write.fbo), U.swap(), X.bind(),
            t.uniform2f(X.uniforms.texelSize, 1 / E, 1 / S), t.uniform1i(X.uniforms.uDivergence, F.texId), r = U.read.texId,
            t.uniform1i(X.uniforms.uPressure, r), t.activeTexture(t.TEXTURE0 + r);
        for (let n = 0; n < e.PRESSURE_ITERATIONS; n++) t.bindTexture(t.TEXTURE_2D, U.read.texture), x(U.write.fbo), U.swap();
        z.bind(), t.uniform2f(z.uniforms.texelSize, 1 / E, 1 / S),
            t.uniform1i(z.uniforms.uPressure, U.read.texId), t.uniform1i(z.uniforms.uVelocity, _.read.texId),
            x(_.write.fbo), _.swap(), N.bind(), t.uniform2f(N.uniforms.texelSize, 1 / E, 1 / S),
            i.supportLinearFiltering || t.uniform2f(N.uniforms.dyeTexelSize, 1 / E, 1 / S), t.uniform1i(N.uniforms.uVelocity, _.read.texId),
            t.uniform1i(N.uniforms.uSource, _.read.texId), t.uniform1f(N.uniforms.dt, n),
            t.uniform1f(N.uniforms.dissipation, e.VELOCITY_DISSIPATION), x(_.write.fbo), _.swap(), t.viewport(0, 0, D, y),
            i.supportLinearFiltering || t.uniform2f(N.uniforms.dyeTexelSize, 1 / D, 1 / y), t.uniform1i(N.uniforms.uVelocity, _.read.texId),
            t.uniform1i(N.uniforms.uSource, A.read.texId), t.uniform1f(N.uniforms.dissipation, e.DENSITY_DISSIPATION),
            x(A.write.fbo), A.swap()
    }
    ...
    function render(n) {
        ...
        null == n && e.TRANSPARENT && C.bind(), e.SHADING ? (B.bind(),
            t.uniform2f(B.uniforms.texelSize, 1 / r, 1 / i), t.uniform1i(B.uniforms.uTexture, A.read.texId)) : (I.bind(),
            t.uniform1i(I.uniforms.uTexture, A.read.texId)), x(n)
    }

    function splat(n, r, i, o, a) {
        t.viewport(0, 0, E, S), P.bind(), t.uniform1i(P.uniforms.uTarget, _.read.texId),
            ...
            t.uniform3f(P.uniforms.color, i, -o, 1), t.uniform1f(P.uniforms.radius, e.SPLAT_RADIUS / 100),
            x(_.write.fbo), _.swap(), t.viewport(0, 0, D, y), t.uniform1i(P.uniforms.uTarget, A.read.texId),
            t.uniform3f(P.uniforms.color, a.r, a.g, a.b), x(A.write.fbo), A.swap()
    }
    ...
    function multipleSplats(e, n = !1) {
        for (let a = 0; a < e; a++) {
            ...
            var r = canvas.width * Math.random(),
                ...
                i = 1e3 * (Math.random() - .5),
                o = 1e3 * (Math.random() - .5);
            n && (r = canvas.width / 2, t = canvas.height / 2, i = 1e3 * (Math.random() - .5),
                o = 1e3 * (Math.random() - .5)), splat(r, t, i, o, e)
        }
    }

    function resizeCanvas() {
        canvas.width == canvas.clientWidth && canvas.height == canvas.clientHeight || (canvas.width = canvas.clientWidth,
            canvas.height = canvas.clientHeight, initFramebuffers())
    }
    update();
    var my_dx = 0,
        my_dy = 0,
    ...
    canvas.addEventListener("mousemove", e => {
            splat(e.offsetX, e.offsetY, 5 * (e.offsetX - my_dx), 10 * (e.offsetY - my_dy), my_color), my_dx = e.offsetX, my_dy = e.offsetY
        }),
        ...
        canvas.addEventListener("touchstart",
            e => {
                ...
                const r = e.targetTouches;
                for (let e = 0; e < r.length; e++)
                    e >= n.length && n.push(new pointerPrototype), n[e].id = r[e].identifier, n[e].down = !0, n[e].x = r[e].pageX,
                    n[e].y = r[e].pageY, n[e].color = generateColor()
