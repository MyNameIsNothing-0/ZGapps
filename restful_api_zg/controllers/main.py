...
from odoo.addons.restful_api_zg.common import extract_arguments, invalid_response, valid_response
...
_routes = ["/api/z3/<model>", "/api/z3/<model>/<id>", "/api/z3/<model>/<id>/<action>"]
...
class APIController(http.Controller):
    ...
    # @validate_token
    @http.route("/webhook/z3/<webhook_id>", type="http", auth="none", methods=["GET","POST"], csrf=False)
    def webhook_endpoint(self,webhook_id,**payload):
        try:
            print(request)
            webhook_id = webhook_id.lower() if webhook_id else ""
            webhook = request.env['odoo.api.webhook'].search([('webhook_url','=',webhook_id)])
            post_data = request.httprequest.data.decode()
            header_data = request.httprequest.headers
            print("_____________",request.httprequest.headers)
            if webhook:
                data = {
                    "name":f"Response - {webhook.webhook_url}",
                    "webhook_id":webhook.id,
                    "response_data": str(post_data),
                    "header_data": str(header_data),
                }
                webhook_res = request.env['odoo.api.webhook.response'].create(data)
                webhook_res.name = f"Response - {webhook.webhook_url} - {webhook_res.id}"
                return valid_response(data)
            else:

                post_data = request.httprequest.data.decode()
                print(post_data)
                return invalid_response("Invalid Webhook id", "Error : %s" % webhook_id)
        except Exception as e:
            return invalid_response("Exception", "Error : %s" % e,)
